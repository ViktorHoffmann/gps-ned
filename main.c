...
void gpsToNed(float lat1, float long1, float alt1, float lat2, float long2, float alt2){
    ...
    float tempLat = originLat;
    float tempLong = originLong;
    ...
    float a = sin(diffLat/2) * sin(diffLong/2) + sin(diffLong/2) * sin(diffLong/2) * cos(originLat) * cos(tempLat); //Haversine Formula
    float c = 2 * atan2(sqrt(a), sqrt(1-a));
    float x = earthRadius * c;

    //Now we calculate direct distance
    diffLat = (currentLat-originLat);      //calc difference in radians between two latitudes and longitudes
    diffLong = (currentLong-originLong);

    a = sin(diffLat/2) * sin(diffLong/2) + sin(diffLong/2) * sin(diffLong/2) * cos(originLat) * cos(currentLat); //Haversine Formula
    c = 2 * atan2(sqrt(a), sqrt(1-a));
    float d = earthRadius * c;

    //Finding missing side of triangle
    float y = sqrt((d*d) - (x*x));
